...
public class IndexModel : PageModel
{
    ...
    public async Task<IActionResult> OnPostUpdatePositionAsync(int productId, int x, int y)
    {
        ...
        var reservedRows = await _ctx.CatalogCafeTitleRows.Select(t => t.MatrixY).ToListAsync(); if (reservedRows.Contains(y)) return BadRequest("No se puede colocar un producto en una fila de t?tulo");
    ...
    public async Task<IActionResult> OnPostCreateTitleAsync(string text, int y, int level = 2)
    {
        ...
        _ctx.CatalogCafeTitleRows.Add(new CatalogCafeTitleRow { Text = text, MatrixY = y, Level = level });
        await _ctx.SaveChangesAsync();
